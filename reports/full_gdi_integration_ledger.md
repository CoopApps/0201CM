...
| --- | --- | --- | --- |
| **Friendlies / pre-season tour** | `friendly.rs` (value type only) | **CONFIRMED ABSENT IN PRODUCTION (playtest 2026-09-17).** `FriendlyMatch::arrange` is never called; `save.friendlies` is always `Vec::new()`. Every club should have July pre-season friendlies (and some a tour) before the league starts — the original always generates them in the first season. This is a PLAYER-VISIBLE gap: the fixture list opens straight onto August league games with an empty pre-season. Arranging AI (~35 exe helpers, `friendly.cpp` 0x005ac250) + a tick calendar slot not written. | **Big** (multi-day port) |
